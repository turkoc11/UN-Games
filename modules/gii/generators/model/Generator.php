...
class Generator extends \app\modules\gii\Generator
{
    ...
    public $messageCategory = 'app_model';
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [['enableI18N'], 'boolean'],
            [['multiLang', 'extend'], 'boolean'],
            [['multiLangArray', 'extendTo'], 'string'],
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $extendTo = ($this->extend) ? $this->generateExtended($modelClassName) : false;
            ...
            if ($this->multiLang && $this->fieldsLang) {
                $mg_params = [];
                ...
                foreach ($tableColumns as $key => $field) {
                    if (!in_array($field->name, $this->fieldsLang) && !$field->isPrimaryKey) {
                ...
                foreach ($tableSchema->columns as $item) {
                    if ($item->name == 'id') {
                        ...
                        $type = $item->type;
                        $mg_params[$item->name] = $mg->$type($item->size)->defaultValue(null);
                    }
                }
            ...
            // extend :
            if ($extendTo) {
                foreach ($extendTo as $module) {
                    $params['module'] = $module;
                    $params['modelClassName'] = $modelClassName;
                    $files[] = new CodeFile(
                        Yii::getAlias('@' . str_replace('\\', '/', $module)) . '/' . $modelClassName . '.php',
                        $this->render('extended.php', $params)
                    );
                    if ($this->multiLang && $this->fieldsLang) {
                        $params['modelClassName'] = $modelClassName. 'Lang';
                        $files[] = new CodeFile(
                            Yii::getAlias('@' . str_replace('\\', '/', $module)) . '/' . $modelClassName . 'Lang.php',
                            $this->render('extended.php', $params)
                        );
                    }
                }
    ...
    /**
     * Get modules for extending class from the specified model class name.
     *
     * @param string $modelClassName model class name
     *
     * @return array names of modules
     */
    protected function generateExtended($modelClassName)
    {
        $modules = [];

        if (!empty($this->extendTo)) {
            $modules = preg_split('/[,\s\;]+/', $this->extendTo);
            if ($modules) {
                $modules = array_filter($modules);
                array_walk($modules, function (&$item) {
                    $item = 'app\\modules\\' . $item . '\\models';
    ...
    /**
     * @return null|mixed Connection the DB connection as specified by [[db]].
     * @throws \yii\base\InvalidConfigException
     */
