...
use Yii;
use app\modules\admin\models\ServiceFaqs;
use app\modules\admin\models\ServiceFaqsSearch;
...
class ServiceFaqsController extends Controller
{
    ...
    public function actionIndex()
    {
		if (Yii::$app->request->isAjax) {
			...
			if (count($keys)) {
				foreach ($keys as $k => $v) {
					if (($model = ServiceFaqs::findOne($v)) !== null) {
						$model->delete();
					}
				}
        ...
        $searchModel = new ServiceFaqsSearch();
...
//            $model->categories_all = $model->categories_all = Category::getSortableArrayTree();
     ...
     * @param integer $id
     * @return ServiceFaqs the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $ml = true)
    {
        if ($ml) {
            $model = ServiceFaqs::find()->where('id = :id', [':id' => $id])->multilingual()->one();
        } else {
            $model = ServiceFaqs::findOne($id);
        }
