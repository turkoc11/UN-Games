...
use Yii;
use app\modules\admin\models\Advantages;
use app\modules\admin\models\AdvantagesSearch;
...
class AdvantagesController extends Controller
{
    ...
    public function actionIndex()
    {
		if (Yii::$app->request->isAjax) {
			...
			if (count($keys)) {
				foreach ($keys as $k => $v) {
					if (($model = Advantages::findOne($v)) !== null) {
						$model->delete();
					}
				}
...
//                ->all(), 'name'));
    ...
    {
        if ($ml) {
            $model = Advantages::find()->where('id = :id', [':id' => $id])->multilingual()->one();
        } else {
