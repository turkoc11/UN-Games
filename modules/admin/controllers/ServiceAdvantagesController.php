...
use Yii;
use app\modules\admin\models\ServiceAdvantages;
use app\modules\admin\models\ServiceAdvantagesSearch;
...
class ServiceAdvantagesController extends Controller
{
    ...
    public function actionIndex()
    {
		if (Yii::$app->request->isAjax) {
			...
			if (count($keys)) {
				foreach ($keys as $k => $v) {
					if (($model = ServiceAdvantages::findOne($v)) !== null) {
						$model->delete();
					}
				}
...
//            $model->categories_all = $model->categories_all = Category::getSortableArrayTree();
     ...
     * @param integer $id
     * @return ServiceAdvantages the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $ml = true)
    {
        if ($ml) {
            $model = ServiceAdvantages::find()->where('id = :id', [':id' => $id])->multilingual()->one();
        } else {
            $model = ServiceAdvantages::findOne($id);
        }
