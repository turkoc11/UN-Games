...
class UserRegister extends \yii\db\ActiveRecord implements IdentityInterface
{
...
*/
    public static function listUsers($is_doctor = false, $is_manager = false){
        ...
        if($is_doctor && $is_manager){
            $users = ArrayHelper::map(UserRegister::find()
                ->where(['status' => 1, 'hidden_user' => 'false'])
                ->andFilterWhere(['or', ['is_manager' => 'true'], ['is_doctor' => 'true']])
                ->orderBy('full_name')->all(), 'id', 'full_name');
        }elseif($is_doctor){
            $users = ArrayHelper::map(UserRegister::find()->where(['status' => 1, 'is_doctor' => 'true', 'hidden_user' => 'false'])->orderBy('full_name')->all(), 'id', 'full_name');
        }elseif($is_manager) {
            $users = ArrayHelper::map(UserRegister::find()->where(['status' => 1, 'is_manager' => 'true', 'hidden_user' => 'false'])->orderBy('full_name')->all(), 'id', 'full_name');
        }else{
            $users = ArrayHelper::map(UserRegister::find()->where(['status' => 1, 'hidden_user' => 'false'])->orderBy('full_name')->all(), 'id', 'full_name');
        }
